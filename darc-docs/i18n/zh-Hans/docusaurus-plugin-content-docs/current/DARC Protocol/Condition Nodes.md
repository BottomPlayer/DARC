...
插件是 DARC 协议的法律。每个插件包含一个条件节点数组以及一个返回类型。每个条件节点可以是条件表达式、逻辑运算符或布尔值。条件数组可以重构为一棵逻辑树，用来表示插件的触发条件。

每个表达式节点由一个操作码（条件节点 ID）和一个或多个参数组成。节点参数的结构定义如下：
...
DARC 协议中有三个逻辑运算符：`AND`、`OR` 和 `NOT`。`AND` 运算符在其所有子节点都为 true 时返回 true；`OR` 运算符在任意一个子节点为 true 时返回 true；`NOT` 运算符在其子节点为 false 时返回 true。`AND` 和 `OR` 运算符必须至少有一个子节点，`NOT` 运算符必须且只能有一个子节点。

在 By-law Script 和 darc.js SDK 中，可以使用 `and()`、`or()` 和 `not()` 这些包装函数来创建逻辑运算符，例如：
...
此外，在 By-law Script 中可以使用 `|` 表示 `OR`，使用 `&` 表示 `AND`，使用 `!` 表示 `NOT`，这些运算符会被解析为对应的条件节点。例如，上述 By-law Script 可以写成：
...
DARC 协议中有两个布尔值：类 `TRUE` 与类 `FALSE`，或包装函数 `boolean_true()` 与 `boolean_false()`。它们用于在条件树中表示布尔节点。
...
更多详情请参阅 [DARC 指令集操作码表(opcode.md)](OpCodes.md)。

2. 占位符保留供未来使用。

3. 区间 [staringValue, endingValue] 为闭区间。例如，若区间为 [1, 3]，则 1、2、3 均被包含。

4. 在 DARC 协议中，所有表达式以大写命名；在 darc.js SDK 和 By-law Script 中，所有表达式以小写命名。例如，DARC 协议中的 `OPERATOR_NAME_EQUALS` 在 darc.js SDK 和 By-law Script 中为 `operator_name_equals`。

| ID | 表达式名称 | 参数 | 备注 | 就绪 |
|------------|------|------|-------| ---- |
| 0  | UNDEFINED |  | 无效操作 |
| 1 | OPERATOR_NAME_EQUALS| STRING_ARRAY[0] operatorName  | 操作者名称与给定字符串完全相同 |
| 2 | OPERATOR_ROLE_EQUALS| UINT256_2DARRAY[0][0] operatorRoleIndex   | 操作者角色索引与 operatorRoleIndex 完全相同 |
| 3 | OPERATOR_ADDRESS_EQUALS   | ADDRESS_2DARRAY[0][0] operatorAddress | 操作者地址等于 operatorAddress|
| 4 | OPERATOR_ROLE_GREATER_THAN | UINT256_2DARRAY[0][0] operatorRoleIndex   | 操作者角色索引大于 operatorRoleIndex|
| 5 | OPERATOR_ROLE_LESS_THAN   | UINT256_2DARRAY[0][0] operatorRoleIndex   | 操作者角色索引小于 operatorRoleIndex|
| 6 | OPERATOR_ROLE_IN_RANGE| UINT256_2DARRAY[0][0] startingOperatorRoleIndex,  UINT256_2DARRAY[0][1] endingOperatorRoleIndex  | 操作者角色索引位于 [startingOperatorRoleIndex, endingOperatorRoleIndex] 区间|
| 7 | OPERATOR_ROLE_IN_LIST | UINT256_2DARRAY[0] operatorRoleIndexArray| 操作者角色索引在 operatorRoleIndexArray 列表中|
| 8 | OPERATOR_TOKEN_X_AMOUNT_GREATER_THAN   | UINT256_2DARRAY[0][0] tokenClass, UINT256_2DARRAY[0][1] amount  | 操作者在 tokenClass 拥有大于 amount 的代币数量|
| 9 | OPERATOR_TOKEN_X_AMOUNT_LESS_THAN | UINT256_2DARRAY[0][0] tokenClass, UINT256_2DARRAY[0][1] amount  ||
| 10 | OPERATOR_TOKEN_X_AMOUNT_IN_RANGE  | UINT256_2DARRAY[0][0] tokenClass, UINT256_2DARRAY[0][1] startingAmount, UINT256_2DARRAY[0][2] endingAmount  | 操作者在 tokenClass 的代币数量位于 [startingAmount, endingAmount] 区间|
| 11 | OPERATOR_TOKEN_X_AMOUNT_EQUALS| UINT256_2DARRAY[0][0] tokenClass, UINT256_2DARRAY[0][1] amount  | 操作者在 tokenClass 的代币数量恰为 amount|
| 12 | OPERATOR_TOKEN_X_PERCENTAGE_GREATER_THAN   | UINT256_2DARRAY[0][0] tokenClass, UINT256_2DARRAY[0][1] percentage  | 操作者在 tokenClass 的代币占比大于 percentage|
| 13 | OPERATOR_TOKEN_X_PERCENTAGE_LESS_THAN | UINT256_2DARRAY[0][0] tokenClass, UINT256_2DARRAY[0][1] percentage  | 操作者在 tokenClass 的代币占比小于 percentage|
| 14 | OPERATOR_TOKEN_X_PERCENTAGE_IN_RANGE  | UINT256_2DARRAY[0][0] tokenClass, UINT256_2DARRAY[0][1] percentageStartingValue,  UINT256_2DARRAY[0][2] percentageEndingValue  | 操作者在 tokenClass 的代币占比位于 [percentageStartingValue, percentageEndingValue] 区间|
| 15 | OPERATOR_TOKEN_X_PERCENTAGE_EQUALS| UINT256_2DARRAY[0][0] tokenClass, UINT256_2DARRAY[0][1] percentage  | 操作者在 tokenClass 的代币占比恰为 percentage|
| 16 | OPERATOR_VOTING_WEIGHT_GREATER_THAN| UINT256_2DARRAY[0][0] amount  | 操作者的投票权重大于 amount|
| 17 | OPERATOR_VOTING_WEIGHT_LESS_THAN  | UINT256_2DARRAY[0][0] amount  | 操作者的投票权重小于 amount|
| 18 | OPERATOR_VOTING_WEIGHT_IN_RANGE   | UINT256_2DARRAY[0][0] startingAmount, UINT256_2DARRAY[0][1] endingAmount  | 操作者的投票权重位于 [startingAmount, endingAmount] 区间|
| 19 | OPERATOR_DIVIDEND_WEIGHT_GREATER_THAN  | UINT256_2DARRAY[0][0] amount  | 操作者的分红权重大于 amount|
| 20 | OPERATOR_DIVIDEND_WEIGHT_LESS_THAN| UINT256_2DARRAY[0][0] amount  | 操作者的分红权重小于 amount|
| 21 | OPERATOR_DIVIDEND_WEIGHT_IN_RANGE | UINT256_2DARRAY[0][0] startingAmount, UINT256_2DARRAY[0][1] endingAmount  | 操作者的分红权重位于 [startingAmount, endingAmount] 区间|
| 22 | Placeholder22  |  |  |
| 23 | Placeholder23  |  |  |
| 24 | Placeholder24  |  |  |
| 25 | OPERATOR_WITHDRAWABLE_CASH_GREATER_THAN | UINT256_2DARRAY[0][0] amount  | 操作者可提取现金大于 amount|
| 26 | OPERATOR_WITHDRAWABLE_CASH_LESS_THAN   | UINT256_2DARRAY[0][0] amount  | 操作者可提取现金小于 amount|
| 27 | OPERATOR_WITHDRAWABLE_CASH_IN_RANGE| UINT256_2DARRAY[0][0] startingAmount, UINT256_2DARRAY[0][1] endingAmount  | 操作者可提取现金位于 [startingAmount, endingAmount] 区间|
| 28 | OPERATOR_WITHDRAWABLE_DIVIDENDS_GREATER_THAN| UINT256_2DARRAY[0][0] amount  | 操作者可提取分红大于 amount|
| 29 | OPERATOR_WITHDRAWABLE_DIVIDENDS_LESS_THAN  | UINT256_2DARRAY[0][0] amount  | 操作者可提取分红小于 amount|
| 30 | OPERATOR_WITHDRAWABLE_DIVIDENDS_IN_RANGE   | UINT256_2DARRAY[0][0] amount  | 操作者可提取分红位于 [startingAmount, endingAmount] 区间|
| 31 | OPERATOR_ADDRESS_IN_LIST  | ADDRESS_2DARRAY[0] addressArray  | 操作者地址位于 addressArray 列表中|
| 32 | Placeholder32  |  |  |
| 33 | Placeholder33  |  |  |
| 34 | Placeholder34  |  |  |
| 35 | Placeholder35  |  |  |
...
| 49 | Placeholder49  |  |  |
| 50 | Placeholder50  |  |  |
| 51 | TIMESTAMP_GREATER_THAN | UINT256_2DARRAY[0][0] timestamp | 当前时间戳大于 timestamp|
| 52 | TIMESTAMP_LESS_THAN | UINT256_2DARRAY[0][0] timestamp | 当前时间戳小于 timestamp|
| 53 | TIMESTAMP_IN_RANGE | UINT256_2DARRAY[0][0] startTimestamp, UINT256_2DARRAY[0][0] endTimestamp | 当前时间戳位于 [startTimestamp, endTimestamp] 区间|
| 54 | DATE_YEAR_GREATER_THAN | UINT256_2DARRAY[0][0] year | 当前年份大于 year|
| 55 | DATE_YEAR_LESS_THAN | UINT256_2DARRAY[0][0] year | 当前年份小于 year|
| 56 | DATE_YEAR_IN_RANGE | UINT256_2DARRAY[0][0] startYear, UINT256_2DARRAY[0][0] endYear | 当前年份位于 [startYear, endYear] 区间|
| 57 | DATE_MONTH_GREATER_THAN | UINT256_2DARRAY[0][0] month | 当前月份大于 month|
| 58 | DATE_MONTH_LESS_THAN | UINT256_2DARRAY[0][0] month | 当前月份小于 month|
| 59 | DATE_MONTH_IN_RANGE | UINT256_2DARRAY[0][0] startMonth, UINT256_2DARRAY[0][0] endMonth | 当前月份位于 [startMonth, endMonth] 区间|
| 60 | DATE_DAY_GREATER_THAN | UINT256[0][0] day | 当前日大于 day|
| 61 | DATE_DAY_LESS_THAN | UINT256[0][0] day | 当前日小于 day|
| 62 | DATE_DAY_IN_RANGE | UINT256[0][0] startDay, UINT256[0][0] endDay | 当前日位于 [startDay, endDay] 区间|
| 63 | DATE_HOUR_GREATER_THAN | uint256 hour | 当前小时大于 hour|
| 64 | DATE_HOUR_LESS_THAN | uint256 hour | 当前小时小于 hour|
| 65 | DATE_HOUR_IN_RANGE | uint256 startHour, uint256 endHour | 当前小时位于 [startHour, endHour] 区间|
| 66 | ADDRESS_VOTING_WEIGHT_GREATER_THAN | UINT256_2DARRAY[0][0] amount, ADDRESS_2DARRAY[0][0] address  | 该地址的投票权重大于 amount|
| 67 | ADDRESS_VOTING_WEIGHT_LESS_THAN  | UINT256_2DARRAY[0][0] amount, ADDRESS_2DARRAY[0][0] address  | 该地址的投票权重小于 amount|
| 68 | ADDRESS_VOTING_WEIGHT_IN_RANGE   | UINT256_2DARRAY[0][0] amount, ADDRESS_2DARRAY[0][0] address  | 该地址的投票权重位于 [startingAmount, endingAmount] 区间|
| 69 | ADDRESS_DIVIDEND_WEIGHT_GREATER_THAN  | UINT256_2DARRAY[0][0] amount, ADDRESS_2DARRAY[0][0] address  | 该地址的分红权重大于 amount|
| 70 | ADDRESS_DIVIDEND_WEIGHT_LESS_THAN| UINT256_2DARRAY[0][0] amount, ADDRESS_2DARRAY[0][0] address  | 该地址的分红权重小于 amount|
...
| 81 | TOTAL_CASH_GREATER_THAN | UINT256_2DARRAY[0][0] amount  || 未就绪，请勿使用
| 82 | TOTAL_CASH_LESS_THAN   | UINT256_2DARRAY[0][0] amount  || 未就绪，请勿使用
| 83 | TOTAL_CASH_IN_RANGE| UINT256_2DARRAY[0][0] amount  || 未就绪，请勿使用
| 84 | TOTAL_CASH_EQUALS| UINT256_2DARRAY[0][0] amount  || 未就绪，请勿使用
...
| 179 | Placeholder179  |  |  |
| 180 | Placeholder180  |  |  |
| 181 | EXTERNAL_CALL_UINT256_RESULT_EQUALS | ADDRESS_2DARRAY[0][0] externalAddress, BYTES encodedParameters, UINT256_2DARRAY[0][0] value || 未就绪，请勿使用
| 182 | EXTERNAL_CALL_UINT256_RESULT_GREATER_THAN | sADDRESS_2DARRAY[0][0] externalAddress, BYTES encodedParameters, UINT256_2DARRAY[0][0] value || 未就绪，请勿使用
| 183 | EXTERNAL_CALL_UINT256_RESULT_LESS_THAN | ADDRESS_2DARRAY[0][0] externalAddress, BYTES encodedParameters, UINT256_2DARRAY[0][0] value || 未就绪，请勿使用
| 184 | EXTERNAL_CALL_UINT256_RESULT_IN_RANGE | ADDRESS_2DARRAY[0][0] externalAddress, BYTES encodedParameters, UINT256_2DARRAY[0][0] minValue, UINT256_2DARRAY[0][1] maxValue || 未就绪，请勿使用
| 185 | EXTERNAL_CALL_STRING_RESULT_EQUALS | string external, string method, string[] args, string expectedValue || 未就绪，请勿使用
| 186 | Placeholder186  |  |  |
| 187 | Placeholder187  |  |  |
| 188 | Placeholder188  |  |  |
| 189 | Placeholder189  |  |  |
...
| 299 | Placeholder299  |  |  |
| 300 | Placeholder300  |  |  |
| 301 | ENABLE_ANY_BEFORE_OP_PLUGIN_INDEX_IN_LIST | UINT256_2DARRAY[0] pluginIndexList | 仅用于检查 "BATCH_ENABLE_PLUGINS" 操作。
| 302 | ENABLE_ANY_AFTER_OP_PLUGIN_INDEX_IN_LIST | UINT256_2DARRAY[0] pluginIndexList | 仅用于检查 "BATCH_ENABLE_PLUGINS" 操作。
| 303 | ENABLE_EACH_BEFORE_OP_PLUGIN_INDEX_IN_LIST | UINT256_2DARRAY[0][0] startPluginIndex | 仅用于检查 "BATCH_ENABLE_PLUGINS" 操作。
| 304 | ENABLE_EACH_AFTER_OP_PLUGIN_INDEX_IN_LIST | UINT256_2DARRAY[0] pluginIndexList | 仅用于检查 "BATCH_ENABLE_PLUGINS" 操作。
| 305 | DISABLE_ANY_BEFORE_OP_PLUGIN_INDEX_IN_LIST | UINT256_2DARRAY[0] pluginIndexList | 仅用于检查 "BATCH_DISABLE_PLUGINS" 操作。
| 306 | DISABLE_ANY_AFTER_OP_PLUGIN_INDEX_IN_LIST | UINT256_2DARRAY[0] pluginIndexList | 仅用于检查 "BATCH_DISABLE_PLUGINS" 操作。 
| 307 | DISABLE_EACH_BEFORE_OP_PLUGIN_INDEX_IN_LIST | UINT256_2DARRAY[0] pluginIndexList | 仅用于检查 "BATCH_DISABLE_PLUGINS" 操作。
| 308 | DISABLE_EACH_AFTER_OP_PLUGIN_INDEX_IN_LIST | UINT256_2DARRAY[0] pluginIndexList | 仅用于检查 "BATCH_DISABLE_PLUGINS" 操作。
| 309 | ENABLE_ANY_BEFORE_OP_PLUGIN_INDEX_IN_RANGE | UINT256_2DARRAY[0][0] startPluginIndex, UINT256_2DARRAY[0][1] endPluginIndex | 仅用于检查 "BATCH_ENABLE_PLUGINS" 操作。
| 310 | ENABLE_ANY_AFTER_OP_PLUGIN_INDEX_IN_RANGE | UINT256_2DARRAY[0][0] startPluginIndex, UINT256_2DARRAY[0][1] endPluginIndex | 仅用于检查 "BATCH_ENABLE_PLUGINS" 操作。
| 311 | ENABLE_EACH_BEFORE_OP_PLUGIN_INDEX_IN_RANGE | UINT256_2DARRAY[0][0] startPluginIndex, UINT256_2DARRAY[0][1] endPluginIndex | 仅用于检查 "BATCH_ENABLE_PLUGINS" 操作。
| 312 | ENABLE_EACH_AFTER_OP_PLUGIN_INDEX_IN_RANGE | UINT256_2DARRAY[0][0] startPluginIndex, UINT256_2DARRAY[0][1] endPluginIndex | 仅用于检查 "BATCH_ENABLE_PLUGINS" 操作。
| 313 | DISABLE_ANY_BEFORE_OP_PLUGIN_INDEX_IN_RANGE | UINT256_2DARRAY[0][0] startPluginIndex, UINT256_2DARRAY[0][1] endPluginIndex | 仅用于检查 "BATCH_DISABLE_PLUGINS" 操作。 
| 314 | DISABLE_ANY_AFTER_OP_PLUGIN_INDEX_IN_RANGE | UINT256_2DARRAY[0][0] startPluginIndex, UINT256_2DARRAY[0][1] endPluginIndex | 仅用于检查 "BATCH_DISABLE_PLUGINS" 操作。
| 315 | DISABLE_EACH_BEFORE_OP_PLUGIN_INDEX_IN_RANGE | UINT256_2DARRAY[0][0] startPluginIndex, UINT256_2DARRAY[0][1] endPluginIndex | 仅用于检查 "BATCH_DISABLE_PLUGINS" 操作。
| 316 | DISABLE_EACH_AFTER_OP_PLUGIN_INDEX_IN_RANGE | UINT256_2DARRAY[0][0] startPluginIndex, UINT256_2DARRAY[0][1] endPluginIndex | 仅用于检查 "BATCH_DISABLE_PLUGINS" 操作。
| 317 | ARE_ALL_PLUGINS_BEFORE_OPERATION |  | 可用于检查 "BATCH_ENABLE_PLUGINS"、"BATCH_DISABLE_PLUGINS"、"BATCH_ADD_PLUGINS" 和 "BATCH_ADD_AND_ENABLE_PLUGINS" 操作。
| 318 | ARE_ALL_PLUGINS_AFTER_OPERATION |  | 可用于检查 "BATCH_ENABLE_PLUGINS"、"BATCH_DISABLE_PLUGINS"、"BATCH_ADD_PLUGINS" 和 "BATCH_ADD_AND_ENABLE_PLUGINS" 操作。
| 319 | IS_ANY_PLUGIN_BEFORE_OPERATION |  | 可用于检查 "BATCH_ENABLE_PLUGINS"、"BATCH_DISABLE_PLUGINS"、"BATCH_ADD_PLUGINS" 和 "BATCH_ADD_AND_ENABLE_PLUGINS" 操作。
| 320 | IS_ANY_PLUGIN_AFTER_OPERATION |  | 可用于检查 "BATCH_ENABLE_PLUGINS"、"BATCH_DISABLE_PLUGINS"、"BATCH_ADD_PLUGINS" 和 "BATCH_ADD_AND_ENABLE_PLUGINS" 操作。
...
| 459 | Placeholder459  |  |  |
| 460 | Placeholder460  |  |  |
| 461 | TOKEN_X_OP_ANY_PRICE_GREATER_THAN | UINT256_2DARRAY[0][0] tokenClass, UINT256_2DARRAY[0][1] price | 适用于以下三个操作："BATCH_PAY_TO_MINT_TOKENS"、"BATCH_PAY_TO_TRANSFER_TOKENS" 和 "BATCH_BURN_TOKENS_AND_REFUND"。
| 462 | TOKEN_X_OP_ANY_PRICE_LESS_THAN | UINT256_2DARRAY[0][0] tokenClass, UINT256_2DARRAY[0][1] price | 适用于以下三个操作："BATCH_PAY_TO_MINT_TOKENS"、"BATCH_PAY_TO_TRANSFER_TOKENS" 和 "BATCH_BURN_TOKENS_AND_REFUND"。
| 463 | TOKEN_X_OP_ANY_PRICE_IN_RANGE | UINT256_2DARRAY[0][0] tokenClass, UINT256_2DARRAY[0][1] startPrice, UINT256_2DARRAY[0][2] endPrice | 适用于以下三个操作："BATCH_PAY_TO_MINT_TOKENS"、"BATCH_PAY_TO_TRANSFER_TOKENS" 和 "BATCH_BURN_TOKENS_AND_REFUND"。
| 464 | TOKEN_X_OP_ANY_PRICE_EQUALS | UINT256_2DARRAY[0][0] tokenClass, UINT256_2DARRAY[0][1] price | 适用于以下三个操作："BATCH_PAY_TO_MINT_TOKENS"、"BATCH_PAY_TO_TRANSFER_TOKENS" 和 "BATCH_BURN_TOKENS_AND_REFUND"。
| 465 | TOKEN_X_OP_ANY_PRICE_GREATER_THAN_EXTERNAL_VALUE_UINT256 | UINT256_2DARRAY[0][0] tokenClass, ADDRESS_2DARRAY[0][0] externalContractAddress, BYTES abiEncodedParameters | 适用于以下三个操作："BATCH_PAY_TO_MINT_TOKENS"、"BATCH_PAY_TO_TRANSFER_TOKENS" 和 "BATCH_BURN_TOKENS_AND_REFUND"。
| 466 | TOKEN_X_OP_ANY_PRICE_LESS_THAN_EXTERNAL_VALUE_UINT256 | UINT256_2DARRAY[0][0] tokenClass, ADDRESS_2DARRAY[0][0] externalContractAddress, BYTES abiEncodedParameters | 适用于以下三个操作："BATCH_PAY_TO_MINT_TOKENS"、"BATCH_PAY_TO_TRANSFER_TOKENS" 和 "BATCH_BURN_TOKENS_AND_REFUND"。
| 467 | TOKEN_X_OP_ANY_PRICE_EQUALS_EXTERNAL_VALUE_UINT256 | UINT256_2DARRAY[0][0] tokenClass, ADDRESS_2DARRAY[0][0] externalContractAddress, BYTES abiEncodedParameters | 适用于以下三个操作："BATCH_PAY_TO_MINT_TOKENS"、"BATCH_PAY_TO_TRANSFER_TOKENS" 和 "BATCH_BURN_TOKENS_AND_REFUND"。
| 468 | Placeholder468  |  |  |
| 469 | Placeholder469  |  |  |
| 470 | Placeholder470  |  |  |
| 471 | Placeholder471  |  |  |
...
| 499 | Placeholder499  |  |  |
| 500 | Placeholder500 |  |  |
| 501 | CREATE_TOKEN_CLASSES_ANY_TOKEN_DIVIDEND_WEIGHT_GREATER_THAN | UINT256_2DARRAY[0][0] weight | 适用于 "BATCH_CREATE_TOKEN_CLASSES" 操作。
| 502 | CREATE_TOKEN_CLASSES_ANY_TOKEN_DIVIDEND_WEIGHT_LESS_THAN | UINT256_2DARRAY[0][0] weight | 适用于 "BATCH_CREATE_TOKEN_CLASSES" 操作。
| 503 | CREATE_TOKEN_CLASSES_ANY_TOKEN_DIVIDEND_WEIGHT_IN_RANGE | UINT256_2DARRAY[0][0]startWeight, UINT256_2DARRAY[0][1] endWeight | 适用于 "BATCH_CREATE_TOKEN_CLASSES" 操作。
| 504 | CREATE_TOKEN_CLASSES_ANY_TOKEN_DIVIDEND_WEIGHT_EQUALS | UINT256_2DARRAY[0][0] weight | 适用于 "BATCH_CREATE_TOKEN_CLASSES" 操作。
| 505 | CREATE_TOKEN_CLASSES_ANY_VOTING_WEIGHT_GREATER_THAN | UINT256_2DARRAY[0][0] weight | 适用于 "BATCH_CREATE_TOKEN_CLASSES" 操作。
| 506 | CREATE_TOKEN_CLASSES_ANY_VOTING_WEIGHT_LESS_THAN | UINT256_2DARRAY[0][0] weight | 适用于 "BATCH_CREATE_TOKEN_CLASSES" 操作。
| 507 | CREATE_TOKEN_CLASSES_ANY_VOTING_WEIGHT_IN_RANGE | UINT256_2DARRAY[0][0]  startWeight, UINT256_2DARRAY[0][1] endWeight | 适用于 "BATCH_CREATE_TOKEN_CLASSES" 操作。
| 508 | CREATE_TOKEN_CLASSES_ANY_VOTING_WEIGHT_EQUALS | UINT256_2DARRAY[0][0] weight | 适用于 "BATCH_CREATE_TOKEN_CLASSES" 操作。
| 509 | Placeholder509 |  |  |
| 510 | Placeholder510 |  |  |
| 511 | Placeholder511 |  |  |
| 512 | Placeholder512 |  |  |
